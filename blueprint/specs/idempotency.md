...
- `POST /checkin/batch` — **bắt buộc** (mỗi item trong batch có `idempotencyKey` riêng).
- `POST /workshops/{id}/pdf` — không dùng interceptor idempotency hiện tại; an toàn retry dựa trên cache SHA-256 của file.
...
- **Yêu cầu**: header bắt buộc cho cả batch + mỗi item có `idempotencyKey`.
...
- Không dùng `@Idempotent` trong code hiện tại. Cache theo SHA-256 file giúp upload lại cùng PDF dùng lại summary và không gọi Gemini lại.
